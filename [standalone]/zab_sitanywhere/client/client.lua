local sitting = false
local standUpCoords = nil
local standUpHeading = nil
...
if config.ox_target then
	local options =
	{
	    {
	    	-- Sit
	        label = config.targetName, name = "nvsit", icon = config.targetIcon, iconColor = "magenta", distance = 1.5,
	        canInteract = function() if not sitting then return true else return false end end,
	    ...
	    {
	    	-- Stand Up
	        label = config.targetNameStandUp, name = "nvstandup", icon = config.targetIcon, iconColor = "magenta", distance = 1.5,
	        canInteract = function() if sitting then return true else return false end end,
...
function sit(entity,newCoords)
    ...
    -- Store the position where player clicked to sit
    standUpCoords = GetEntityCoords(playerPed)
    standUpHeading = GetEntityHeading(playerPed)
...
RegisterCommand('neveradev:sit:stand_up', function()
    if sitting then
        ...
        -- Cancel sitting scenario
        ClearPedTasksImmediately(playerPed)
        
        -- Return to original position
        if standUpCoords then
            SetEntityCoords(playerPed, standUpCoords.x, standUpCoords.y, standUpCoords.z, false, false, false, false)
            SetEntityHeading(playerPed, standUpHeading)
        end
        
        -- Unfreeze chair and reset attachments
