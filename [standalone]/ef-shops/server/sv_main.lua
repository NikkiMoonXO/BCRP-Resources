	...
	for shopID, shopData in pairs(LOCATIONS) do
		...
		for item, data in pairs(PRODUCTS[shopData.shopItems]) do
			shopProducts[#shopProducts + 1] = {
				id = tonumber(item),
				name = data.name,
				price = config.fluctuatePrices and (math.round(data.price * (math.random(100, 100) / 100))) or data.price or 0, -- Price fluctuation algorithm from ox_inventory https://github.com/overextended/ox_inventory/blob/e3a6b905a1b8bdbd3066d012522cf5993466d166/modules/shops/server.lua#L32
